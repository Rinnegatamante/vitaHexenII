...
void D_Sky_uv_To_st_V2 (int u, int v, fixed16_t *s, fixed16_t *t)
{
	int	wu, wv; 
	int 	temp, temp2;
	float   temp3;
	...
	wu = (int) (8192 * (u-((int)vid.width>>1))) / temp;
	wv = (int) (8192 * (((int)vid.height>>1)-v)) / temp;
	...
	temp3 = skytime*skyspeed;	// TODO: add D_SetupFrame & set this there
	
	*s = (int)((temp3 + temp2*end[0]) * 0x10000);
	*t = (int)((temp3 + temp2*end[1]) * 0x10000);
}
...
void D_DrawSkyScans8_V2 (espan_t *pspan)
{
	...
	// calculate the initial s & t
		...
		do
		{
			...
			// calculate s and t at far end of span,
			// calculate s and t steps across span by shifting
				D_Sky_uv_To_st_V2 (u, v, &snext, &tnext);
