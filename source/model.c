...
void *Mod_LoadSpriteFrame (model_t *mod, void *pin, mspriteframe_t **ppframe)
{
	...
	pspriteframe = Hunk_AllocName (sizeof (mspriteframe_t) + size*r_pixbytes, mod->name);
	...
	for (i=0 ; i<numframes ; i++)
	{
		...
		if (frametype == SPR_SINGLE)
		{
			pframetype = (dspriteframetype_t *)
					Mod_LoadSpriteFrame (mod, pframetype + 1, &psprite->frames[i].frameptr);
		
		}
		else
		{
			pframetype = (dspriteframetype_t *)
					Mod_LoadSpriteGroup (mod, pframetype + 1, &psprite->frames[i].frameptr);
		}
	}
	mod->type = mod_sprite;
}
